    ...
    
{
    ...
    //  BEFORE WE CREATE A NEW FILE, WE MUST CHECK IF ONE OF THIS CHARACTER SLOT ALREADY EXISTS (MAX 10 CHARACTER SLOTS)
    ...
    // USED TO CREATE A SAVE FILE UPON STARTING A NEW GAME OR SAVING AN EXISTING GAME
    public void CreateNewCharacterSaveFile(CharacterSaveData characterSaveData)
    {
        // MAKE A PATH TO SAVE FILE LOCALLY
    ...
    public CharacterSaveData LoadSaveFile()
    {
        ...
        // MAKE A PATH TO LOAD FILE LOCALLY
        ...
        if (File.Exists(loadPath))
        {
            try
            {
                ...
                // DESERIALIZE THE DATA FROM JSON BACK TO UNITY C# FORMAT
                characterData = JsonUtility.FromJson<CharacterSaveData>(dataToLoad);
            }
            catch (Exception ex)
            {
                Debug.Log("FILE IS BLANK, OR SOME OTHER ERROR");
            }
        }
